            ...
            <v-container fluid>
                ...
                <v-form ref="form" v-model="formIsValid" lazy-validation>
                    <v-layout>
                        ...
                        <v-flex xs12 md4>
                            <v-container fluid>
                                <v-text-field v-model="addPromocodeFields.code" :rules="formValidationRules.codeRules" label="Code" required></v-text-field>
                                <v-text-field v-model="addPromocodeFields.discount" :rules="formValidationRules.discountRules" label="Discount(%)" required></v-text-field>
                            </v-container>
                        </v-flex>
                        <v-flex xs12 md4>
                        </v-flex>
                    ...
                    <v-container fluid>
                        <b-button @click="addPromocodeSubmitted" class="mr-2 mb-2" :variant="formIsValid? 'success':'danger'" :disabled="formIsValid ? false:true">{{formIsValid ? 'Add Promocode': 'Invalid Inputs'}}</b-button>
                    </v-container>
                </v-form>
                </card>
            </v-container>
